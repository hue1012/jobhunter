...
public class SubscriberController {
    ...
    public ResponseEntity<Subscriber> update(@RequestBody Subscriber subsRequest) throws IdInvalidException {
        // check id
        Optional<Subscriber> subsDBOptional = this.subscriberService.findById(subsRequest.getId());
        if (subsDBOptional.isEmpty()) {
            throw new IdInvalidException("Id " + subsRequest.getId() + " không tồn tại");
        }

        return ResponseEntity.ok().body(this.subscriberService.update(subsDBOptional.get(), subsRequest));
    }
